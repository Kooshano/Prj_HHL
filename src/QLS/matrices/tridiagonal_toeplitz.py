...
class TridiagonalToeplitz(LinearSystemMatrix):
    ...
    def evolution_time(self, evolution_time: float) -> None:
        ...
        self.trotter_steps = int(
            np.ceil(
                np.sqrt(
                    ((evolution_time * np.abs(self.off_diag)) ** 3) / 2 / self.tolerance
                )
            )
        )
    ...
    def eigs_bounds(self) -> Tuple[float, float]:
        ...
        if abs(self.main_diag) < 2 * abs(self.off_diag):
            optimal_index = int(
                np.arccos(self.main_diag / 2 / self.off_diag) / np.pi * (n_b + 1)
            )
    ...
    def _reset_registers(self, num_state_qubits: int) -> None:
        ...
        self.qregs = [qr_state]
        self._ancillas: List[AncillaRegister] = []
        ...
        def control(num_ctrl_qubits=1, label=None, ctrl_state=None):
            ...
            for i in range(0, self.num_state_qubits - 1):
                ...
                if len(q_controls) > 1:
                    ...
                    qc_control.cu(
                        -2 * theta, 3 * np.pi / 2, np.pi / 2, 0, q_controls[0], qr[i]
                    )
